...
TYPE particle
  REAL(REAL64), DIMENSION(2) :: pos, vel
END TYPE
...
SUBROUTINE problem_setup(rundat, axis, part, rho, dx, dy)
  ...
  IF (rundat%prob == 'null') THEN
    rho = 0
    part%pos = (/ 0.0_REAL64, 0.0_REAL64 /)
    part%vel = (/ 0.1_REAL64, 0.1_REAL64 /)
  ELSE IF (rundat%prob == 'single') THEN
    DO j = 1, rundat%nx
      ...
      END DO
    END DO
    part%pos = (/ 0.1_REAL64, 0.0_REAL64 /)
    part%vel = (/ 0.0_REAL64, 0.0_REAL64 /)
  ELSE
    DO j = 1, rundat%nx
      DO i = 1, rundat%ny
      ...
      END DO
    END DO
    part%pos = (/ 0.0_REAL64, 0.5_REAL64 /)
    part%vel = (/ 0.0_REAL64, 0.0_REAL64 /)
  END IF

  ! Print problem setup
  !PRINT '(A)', "Problem initialised, charge density on grid calculated."
  PRINT '(2(A,F4.1))', "Initial particle position is: ", part%pos(1), ", ", &
    part%pos(2)
  PRINT '(2(A,F4.1))', "Initial particle velocity is: ", part%vel(1), ", ", &
    part%vel(2)
